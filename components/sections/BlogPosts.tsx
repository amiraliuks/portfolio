...
import type { BlogLanguage, BlogPost } from "@/types/types";
import { tinyBlurDataURL } from "@/lib/image";
import { trackEvent } from "@/lib/analytics";

interface BlogPostsProps {
  posts: BlogPost[];
  basePath?: "/blog" | "/writeups";
...
type PresentablePost = BlogPost & {
  displayTags: string[];
  languageCodes: BlogLanguage[];
};

function getLanguageCodes(post: BlogPost): BlogLanguage[] {
  const available = post.metadata.availableLanguages ?? [];
  const codes = (["en", "al"] as BlogLanguage[]).filter((language) =>
    available.includes(language)
  );

  return codes.length > 0 ? codes : [post.metadata.language ?? "en"];
}

function getLanguageName(language: BlogLanguage) {
  return language === "al" ? "Albanian" : "English";
}

function getCollectionBadge(basePath: "/blog" | "/writeups", tags: string[]) {
  if (basePath === "/writeups") return "CTF";
  if (tags.some((tag) => /cve|security|research|iot/i.test(tag))) return "Research";
  return "Blog";
}
...
function LanguageBadges({
  ...
  return (
    <div
      ...
      aria-label={`Available in ${languages.map(getLanguageName).join(" and ")}`}
    >
      {languages.map((language) => (
        <span
          key={language}
          title={getLanguageName(language)}
          className="border border-border/80 px-1.5 py-0.5 font-mono text-[10px] uppercase leading-none text-muted-foreground"
        >
          {language}
        </span>
      ))}
...
export function BlogPosts({
  ...
  featuredLabel = "Top 2 recent posts",
  emptyLabel = "No blog posts matched your current search/filter.",
  hiddenTags = [],
}: BlogPostsProps) {
  ...
  const presentablePosts = useMemo<PresentablePost[]>(() => {
    const hiddenTagSet = new Set(hiddenTags.map((tag) => tag.trim().toLowerCase()));

    return sortedPosts.map((post) => ({
      ...post,
      displayTags: normalizeBlogTags(post.metadata.tags, MAX_POST_TAGS).filter(
        (tag) => !hiddenTagSet.has(tag.trim().toLowerCase())
      ),
      languageCodes: getLanguageCodes(post),
    }));
  }, [hiddenTags, sortedPosts]);
  ...
  return (
    <div className="space-y-9">
      <div className="relative" role="search" aria-label={`Search ${collectionLabel}`}>
        <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <label htmlFor="blog-search" className="sr-only">
          Search {collectionLabel} by title, summary, or tag
        </label>
        <input
          id="blog-search"
          type="search"
          ...
          className="h-11 w-full rounded-none border border-border bg-background pl-10 pr-3 text-sm text-foreground placeholder:text-muted-foreground focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/30"
        />
      </div>
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-2 border-b border-border/60 pb-6">
          <button
            ...
            className={`px-4 py-1.5 text-sm rounded-none border transition
              ${activeTag === null
          ...
          {extraTags.length > 0 && (
            <button
              ...
              className="rounded-none border border-border px-4 py-1.5 text-sm text-muted-foreground transition hover:bg-accent hover:text-accent-foreground"
            >
      ...
      {showFeatured && (
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">{featuredLabel}</span>
          </div>
            ...
            {featuredPosts.map((post, featuredIndex) => {
              return (
                <Link
                  ...
                  className="group overflow-hidden rounded-none border border-border bg-card/70 transition duration-300 hover:-translate-y-0.5 hover:border-border hover:bg-accent/25"
                  onClick={() =>
                ...
                >
                  ...
                  <div className="space-y-3 p-5">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      <span className="border border-border px-1.5 py-0.5 font-mono text-[10px] uppercase leading-none text-muted-foreground">
                        {getCollectionBadge(basePath, post.displayTags)}
                      </span>
                    ...
                    <LanguageBadges languages={post.languageCodes} />
                  </div>
                </Link>
              );
            })}
        ...
        {visiblePosts.map((post, index) => {
          return (
            <Link
              key={post.slug}
              href={`${basePath}/${post.slug}`}
              className="group block rounded-none border border-transparent px-1 py-6 transition hover:border-border hover:bg-accent/25 sm:px-3"
              onClick={() =>
                trackEvent("blog_post_open", {
                  source: "blog_list",
                  slug: post.slug,
            ...
            >
              <div className="grid items-start gap-5 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:gap-6">
                ...
                <div className="space-y-2">
                  ...
                  {post.displayTags.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-1">
                      <span className="rounded-none border border-border bg-muted/40 px-2 py-0.5 text-xs text-muted-foreground">
                        {getCollectionBadge(basePath, post.displayTags)}
                      </span>
                ...
                <div className="space-y-2 pt-1 text-left text-xs whitespace-nowrap sm:text-right">
                  ...
                  <LanguageBadges languages={post.languageCodes} align="right" />
                </div>
              </div>
            </Link>
          );
      ...
      {filteredBlogs.length === 0 && (
        <div className="rounded-none border border-border bg-muted/25 p-6 text-center text-sm text-muted-foreground">
          {emptyLabel}
        </div>
      )}
      ...
      {hasMore && (
        <div className="flex flex-col items-center gap-3 pt-2">
          <button
            ...
            className="rounded-none border border-border px-5 py-2 text-sm text-foreground transition hover:bg-accent"
          >
            Load more posts
          </button>
