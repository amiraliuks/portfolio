...
function getRelatedPosts(currentPost: BlogPost) {
  ...
  const scored = getWriteupListingPosts()
...
export async function generateMetadata({ params }: WriteupPageProps) {
  ...
  const ogTitle = `${title} | Writeups | Amir Aliu`;
  const ogImage =
    absoluteImage ??
  ...
  const languageAlternates = getBlogHreflangAlternates(post as BlogPost, baseUrl, "/writeups");
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/writeups/${post.slug}`,
...
export default async function Writeup({ params }: WriteupPageProps) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: "Writeups",
        item: "https://amiraliu.vercel.app/writeups",
      },
      {
        "@type": "ListItem",
        position: 3,
        name: post.metadata.title,
        item: `${baseUrl}/writeups/${post.slug}`,
      },
    ],
  };
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: `${baseUrl}/writeups/${post.slug}`,
    keywords: (post.metadata.tags ?? []).join(", "),
    inLanguage: currentLanguage === "al" ? "sq-AL" : "en-US",
  };

  const postUrl = `${baseUrl}/writeups/${post.slug}`;
    ...
    >
      ...
      {relatedPosts.length > 0 && (
        <section className="mt-12">
          <h2 className="text-lg font-semibold tracking-tight">Related Writeups</h2>
